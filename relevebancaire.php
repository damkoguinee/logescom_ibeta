...
if (isset($_SESSION['pseudo'])) {
  ...
  if ($products['level']>=3) {    
    ...
    <table class="payement" style="width:95%;"><?php 
      ...
      <thead>
        <tr>
          <th height="30" colspan="5"><?="Relevé Bancaire en ".strtoupper($_SESSION['deviseselect']);?> Solde Compte: <?=number_format($montantcredits+$montantdebits,2,',',' ');?>
          <?php
            if (isset($_POST['devise'])) {?>
              <a href="printreleve.php?banque=<?=$_SESSION['banquerel'];?>&devise=<?=$_POST['devise'];?>" target="_blank" ><img  style=" margin-left: 20px; height: 20px; width: 20px;" src="css/img/pdf.jpg"></a><?php 
            }else{
              if (!empty($_SESSION['banquerel'])) {?>
                <a href="printreleve.php?banque=<?=$_SESSION['banquerel'];?>" target="_blank" ><img  style=" margin-left: 20px; height: 20px; width: 20px;" src="css/img/pdf.jpg"></a><?php 
              }
            }?>
          </th>
        </tr>
