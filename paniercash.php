<?php
	...
	if (isset($_GET['delPanier'])) {
		...
		$DB->delete('DELETE FROM validpaie_cash WHERE id = ? and pseudov=?', array($_GET['delPanier'], $_SESSION['idpseudo']));

		$DB->delete('DELETE FROM validvente_cash where pseudop=?', array($_SESSION['idpseudo']));
	...
	if (isset($_GET['desig'])) {
		...
		$prodvalidcverif = $DB->querys('SELECT quantite FROM validpaie_cash where id_produit=? and pseudov=?', array($_GET['idc'], $_SESSION['idpseudo']));

		if (empty($prodvalidcverif)) {
					
			$DB->insert('INSERT INTO validpaie_cash (id_produit, codebvc, quantite, pvente, pseudov, datecmd) VALUES(?, ?, ?, ?, ?, now())', array($_GET['idc'], 1, 1, $_GET['pv'], $_SESSION['idpseudo']));

		}else{

			$qtitesup=$prodvalidcverif['quantite']+1;

			$DB->insert('UPDATE validpaie_cash SET quantite=? where id_produit=? and pseudov=?', array($qtitesup, $_GET['idc'], $_SESSION['idpseudo']));
	...
	if (isset($_GET['scanneur'])) {
		...
		$prodvalidcverif = $DB->querys('SELECT quantite FROM validpaie_cash where id_produit=? and pseudov=?', array($prodstock['id'], $_SESSION['idpseudo']));

		if (empty($prodvalidcverif)) {

			$DB->insert('INSERT INTO validpaie_cash (id_produit, codebvc, quantite, pvente, pseudov, datecmd) VALUES(?, ?, ?, ?, ?, now())', array($prodstock['id'], 1, 1, $prodstock['prix_vente'], $_SESSION['idpseudo']));
		}else{

			$qtitesup=$prodvalidcverif['quantite']+1;

			$DB->insert('UPDATE validpaie_cash SET quantite=? where id_produit=? and pseudov=?', array($qtitesup, $prodstock['id'], $_SESSION['idpseudo']));
	...
	if (isset($_POST['pvente']) or isset($_POST['quantity'])) {
		...
		$DB->insert('UPDATE validpaie_cash SET quantite=?, pvente=? where id=? and pseudov=?', array($_POST['quantity'], $pvente, $_POST['idv'], $_SESSION['idpseudo']));
	}
	$totcomd=0;
	if ($_SESSION['level']<=7) {
		...
		if (isset($_GET['desig']) or isset($_GET['scanneur']) or isset($_POST['quantity']) or isset($_GET['delPanier'])) {		
			...
			$proddirect = $DB->query('SELECT *FROM validpaie_cash where pseudov=?', array($_SESSION['idpseudo']));
			$totcomd=0;
			foreach ($proddirect as $valued) {
				$totcomd+=$valued->quantite*$valued->pvente;
			}

			$prodverifv=$DB->querys('SELECT id from validvente_cash where pseudop=?', array($_SESSION['idpseudo']));

			if (empty($prodverifv)) {

				$DB->insert('INSERT INTO validvente_cash (remise, montantpgnf, montantpeu, montantpus, montantpcfa, virement, cheque, numcheque, banqcheque, pseudop) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?)', array(0, $totcomd, 0, 0, 0, 0, 0, 0, 0, $_SESSION['idpseudo']));
			}else{
			
				$DB->insert('UPDATE validvente_cash SET montantpgnf=? where pseudop=?', array($totcomd, $_SESSION['idpseudo']));
			}
		}
	}
	...
	if (isset($_POST['remise']) or isset($_POST['remisep']) or isset($_POST['gnfpaye']) or isset($_POST['europaye']) or isset($_POST['uspaye']) or isset($_POST['cfapaye'])) {
		...
		if (empty($prodverifv)) {
			...
			$DB->insert('INSERT INTO validvente_cash (remise, montantpgnf, montantpeu, montantpus, montantpcfa, virement, cheque, numcheque, banqcheque, pseudop) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?)', array($remise, $montantgnf, $montanteu, $montantus, $montantcfa, $montantvir, $montantcheq, $numcheque, $banqcheque, $_SESSION['idpseudo']));
		}else{
		
			$DB->insert('UPDATE validvente_cash SET remise=?, montantpgnf=?, montantpeu=?, montantpus=?, montantpcfa=?, virement=?, cheque=?, numcheque=?, banqcheque=? where pseudop=?', array($remise, $montantgnf, $montanteu, $montantus, $montantcfa, $montantvir, $montantcheq, $numcheque, $banqcheque, $_SESSION['idpseudo']));
		}

		if ($_POST['remisep']==0) {
			$DB->insert('UPDATE validvente_cash SET remise=? where pseudop=?', array(0, $_SESSION['idpseudo']));
		}
	...
	$products=$DB->query("SELECT productslist.id as id, validpaie_cash.id as idv, id_produit, validpaie_cash.quantite as quantite, productslist.designation as designation, Marque, pvente, prix_vente as prix_vente, productslist.type as type, productslist.qtiteint as qtiteint, productslist.qtiteintp as qtiteintp FROM validpaie_cash inner join productslist on productslist.id=validpaie_cash.id_produit inner join `".$nomtab."` on idprod=productslist.id where pseudov='{$_SESSION['idpseudo']}' order by(validpaie_cash.id) desc ");

	$prodvente = $DB->querys('SELECT remise, montantpgnf, montantpeu, montantpus, montantpcfa, virement, cheque, numcheque, banqcheque FROM validvente_cash where pseudop=?', array($_SESSION['idpseudo']));?>
					...
					<div class="container-fluid">
						<div class="row">
							<div class="col-sm-12 col-md-3">
								<input type="text" name="products" id="products"  class="form-control" placeholder="rechercher un produit">
								<div class="bg-danger text-start" id="result-products"></div>
							</div>
