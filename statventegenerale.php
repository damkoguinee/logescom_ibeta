...
foreach ($sql_annees as $row_annees) {
    ...
    $sql_ventes_par_mois = $DB->query("SELECT MONTH(date_cmd)as MOIS, (SUM(Total)/1000) AS PRODUIT_VENTE FROM payement where year(date_cmd)='{$row_annees->ANNEE}' GROUP BY MOIS");
...
// TITRE: texte
$graph->title->Set("Chiffre d'affaires/1 000 ");
