...
if (isset($_SESSION['pseudo'])) {
  ...
  $DB->insert("CREATE TABLE IF NOT EXISTS `".$bdd."`(
    ...
    `bl` varchar(150),
    `nature` varchar(150),
    `etat` varchar(15) DEFAULT 'non paye',
  ...
  if ($_SESSION['level']>=3) {
    ...
    if (isset($_GET['deleteret'])) {
      ...
      $DB->delete("DELETE from achat_fournisseur where numedit='{$_GET['deleteret']}'");
    ...
    if (isset($_POST["valid"])) {
      ...
      if (empty($_POST["client"]) or empty($_POST["montant"])) {?>
        ...
        $numdec = $DB->querys('SELECT max(id) AS id FROM achat_fournisseur');
        $numdec=$numdec['id']+1;
        ...
        $prodverif = $DB->querys("SELECT id FROM achat_fournisseur where bl='{$bl}' ");
        ...
        if (!empty($prodverif['id'])) {?>
          ...
          if (empty($dateop)) {
            ...
            $DB->insert('INSERT INTO achat_fournisseur (numedit, id_client, montant, bl, nature, libelle, devise, lieuvente, dateop) VALUES(?, ?, ?, ?, ?, ?, ?, ?, now())', array($numdec, $client, $montant, $bl, $nature, $motif, $devise, $lieuventeret));

            $DB->insert('INSERT INTO bulletin (nom_client,origine_bull, montant, libelles, numero, devise, caissebul, lieuvente, date_versement) VALUES(?, ?, ?, ?, ?, ?, ?, ?, now())', array($client, "achatfi", $montant, $motif, $numdec, $devise, 1, $lieuventeret));

          }else{ 

            $DB->insert('INSERT INTO achat_fournisseur (numedit, id_client, montant, bl, nature, libelle, devise, lieuvente, dateop) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?)', array($numdec, $client, $montant, $bl, $nature, $motif, $devise, $lieuventeret, $dateop));

            $DB->insert('INSERT INTO bulletin (nom_client,origine_bull, montant, libelles, numero, devise, caissebul, lieuvente, date_versement) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?)', array($client, "achatfi", $montant, $motif, $numdec, $devise, 1, $lieuventeret, $dateop));
    ...
    if (isset($_POST["validup"])) {
        ...
        if (empty($_POST["client"]) or empty($_POST["montant"])) {?>
          ...
          if ($bl==$blinit) {
            ...
            $DB->delete("DELETE from achat_fournisseur where numedit='{$numdec}'");
    ...
    if (isset($_GET['update'])) {
      ...
      $prod = $DB->querys("SELECT *FROM achat_fournisseur where numedit='{$_GET['update']}' ");
      ...
      <div style="overflow: auto">
        ...
        <table class="table table-hover table-bordered table-striped table-responsive text-center align-middle">
          <thead class="sticky-top bg-secondary text-center">
            <tr>
              <th colspan="11"><?="factures fournisseurs";?>
          ...
          <tbody><?php
            if (isset($_GET['search'])) {
              ...
              $products= $DB->query("SELECT etat_achatf as etat, bl, libelle, nature, id_client, montant, devise, dateop, numedit, achat_fournisseur.id as id FROM achat_fournisseur inner join client on id_client=client.id  WHERE (nom_client LIKE ? OR telephone LIKE ? OR bl LIKE ? ) and  lieuvente LIKE ? order by(achat_fournisseur.id) DESC ", array("%".$terme."%", "%".$terme."%", "%".$terme."%", $_SESSION['lieuvente']));
            }elseif (isset($_GET['search_fact'])) {
              $terme = htmlspecialchars($_GET["search_fact"]);
              $products= $DB->query("SELECT etat_achatf as etat, bl, libelle, nature, id_client, montant, devise, dateop, numedit, achat_fournisseur.id as id FROM achat_fournisseur  WHERE (id_client LIKE ?) and  lieuvente LIKE ? order by(achat_fournisseur.id) DESC ", array($terme, $_SESSION['lieuvente']));
            }else{
              $products= $DB->query("SELECT etat_achatf as etat, bl, libelle, nature, id_client, montant, devise, dateop, numedit, achat_fournisseur.id as id FROM achat_fournisseur  WHERE lieuvente='{$_SESSION['lieuvente']}' order by(id) DESC ");
            }   
            ...
            foreach ($products as $keyv=> $product ){
              ...
              <tr>
                ...
                <td class="bg-<?=$bg;?>"><?= strtoupper($product->bl); ?></td>
                <td class="text-start bg-<?=$bg;?>"><?=$product->libelle; ?></td>
                <td class="text-start bg-<?=$bg;?>"><?=$product->nature; ?></td>
                <td class="text-start bg-<?=$bg;?>"><?=strtoupper($panier->nomClient($product->id_client)); ?></td><?php

                $montantgnf+=$product->montant;?>

                <td class="text-end bg-<?=$bg;?>"><?= number_format($product->montant,0,',',' '); ?></td>
                <td class="bg-<?=$bg;?>"><?=$product->devise;?></td>
                <td><?php if ($_SESSION['level']>=1){?><a onclick="return alerteV();" class="btn btn-success" href="achat_fournisseur_reception.php?reception=<?=$product->numedit;?>&bl=<?=$product->bl;?>&idclient=<?=$product->id_client;?>&datef=<?=$product->dateop;?>">Receptionner</a><?php };?></td>

                <td>
                  <?php if ($_SESSION['level']>=6){?><a class="btn btn-warning" href="edition_facture_achat.php?numcmdmodif=<?=$product->id;?>"><i class="fa fa-edit"></i></a><?php };?>

                  <!-- <?php if ($_SESSION['level']>=6 ){?><a class="btn btn-warning" onclick="return alerteV();" href="?update=<?=$product->numedit;?>">Modifier</a><?php };?> -->
                </td>
