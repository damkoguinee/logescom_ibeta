	...
	if ($products['level']>=3) {
    	...
    	$soldea=0;
      	...
      	if (isset($_POST['j1']) or isset($_POST['j2'])) {
        	...
        	$prod =$DB->query("SELECT client.nom_client as client, libelles, numero, montant, date_versement, devise, bulletin.lieuvente as lieuvente, numfact FROM bulletin inner join client on client.id=bulletin.nom_client left join facture on numero=numcmd WHERE bulletin.nom_client='{$client}' and DATE_FORMAT(date_versement, \"%Y%m%d\") >='{$_SESSION['date1']}' and DATE_FORMAT(date_versement, \"%Y%m%d\") <='{$_SESSION['date2']}' ORDER BY (date_versement)");

      	}else{

        	$prod =$DB->query("SELECT bulletin.nom_client as client, libelles, numero, montant, date_versement, devise, bulletin.lieuvente as lieuvente, numfact FROM bulletin inner join client on client.id=bulletin.nom_client left join facture on numero=numcmd WHERE bulletin.nom_client='{$client}' and devise='{$devise}' ORDER BY (date_versement)");
      	}?>
