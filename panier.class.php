<?php
	class panier{
		...
		private $DB;
		public $monnaie=['gnf','eu','us', 'aed'];

		public $modep=['especes','cheque','virement'];
		...
		public function datemax($nombre){
			$dated=date("Y-m-d");
			...
			$datemax = date("Y-m-d", mktime(0, 0, 0, $moisd, $jourd,   $anneed));
		...
		public function nomPersonnelId($nom){
			$nomclient = $this->DB->querys("SELECT *FROM login where identifiant='{$nom}'");
	...
	# FONCTION POUR CALCULER LES CREANCES DU MAGASIN
		...
		public function listeCloture($devise){

			$prod=$this->DB->query("SELECT *FROM caissecloture where devise='{$devise}' order by(id)");			
