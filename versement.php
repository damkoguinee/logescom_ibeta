        ...
        if ($products['level']>=3) {
          ...
          if (isset($_POST["valid"])) {
            ...
            if (empty($_POST["client"]) or empty($_POST["montant"]) or empty($_POST['devise'])) {?>
              ...
              $DB->insert('INSERT INTO banque (id_banque,clientbanque,typeent,origine, montant, libelles, numero, devise, typep, lieuvente, numeropaie, banqcheque, date_versement) VALUES(?, ?, ?, ?, ?,?, ?,?,?, ?, ?, ?, ?)', array($compte,$client,"depot",'client', $montant, $motif, 'dep'.$max, $devise, $payement, $lieuventeret, $numcheque, $banquecheque, $dateop));

              if ($prodclient['typeclient']=='Banque') {

                $DB->insert('INSERT INTO banque (id_banque,clientbanque,typeent,origine, montant, libelles, numero, devise, typep, lieuvente, numeropaie, banqcheque, date_versement) VALUES(?, ?, ?, ?, ?,?, ?,?,?, ?, ?, ?, ?)', array($prodclient['id'],$client,"depot",'client', -$montant, $_POST['motif'], 'dep'.$max, $devise, $payement, $lieuventeret, $numcheque, $banquecheque, $dateop));
              }
