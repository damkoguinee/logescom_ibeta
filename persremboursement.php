      ...
      <div class="row"><?php 
        ...
        <div class="col col-sm-12 col-md-10" style="overflow: auto;"><?php
          ...
          if (isset($_GET['deleteret'])) {
            ...
            $DB->delete("DELETE from persremboursement where numdep='{$_GET['deleteret']}'");
          ...
          if (isset($_GET['ajoutdep']) or isset($_POST['categins']) or isset($_GET["categ"])) {
            ...
            <form class="form bg-light p-1" method="POST" >
              ...
              <div class="row my-2">
                ...
                <div class="col-sm-12 col-md-6">
                  <input class="form-control" id="numberconvert" type="number" name="montant" min="0" max="<?=$credit;?>" required="">
                </div>
                <div class="col-sm-12 col-md-6">
              ...
              <div class="row mb-1">
                <div class="col-sm-12 col-md-4">
                  <label class="form-label">Compte de depôt</label>
          ...
          if (isset($_POST['valid'])){         
            ...
            if ($_POST['montant']<0){?>
              ...
              if (!empty($_POST['client']) and !empty($_POST['categorie']) and !empty($_POST['compte'])) {
                ...
                $numdec = $DB->querys('SELECT max(id) AS id FROM persremboursement');
                $numdec=$numdec['id']+1;
                ...
                $DB->insert('INSERT INTO persremboursement (numdep, client, montant, devise, payement, coment, cdepot, lieuvente, exect, date_payement) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',array('rembp'.$numdec, $personnel, $montant, $devise, $payement, $motif, $compte, $lieuventeret, $_SESSION['idpseudo'], $dateop));
                $DB->insert('INSERT INTO banque (id_banque,clientbanque, montant, libelles, numero, devise, lieuvente,typeent,origine, date_versement) VALUES(?, ?,?,?,?, ?, ?, ?, ?, ?)', array($compte,$personnel, $montant, $motif, 'rembp'.$numdec, $devise, $lieuventeret,"remboursbon","personnel", $dateop));                
                $DB->insert('INSERT INTO bulletin (nom_client, montant, libelles, numero, devise, caissebul, lieuvente, type, date_versement) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?)', array($personnel, $montant, "remboursement", 'rembp'.$numdec, $devise, $compte, $lieuventeret,"personnel", $dateop));?>
                <div class="alert alert-success">Remboursement enregistré avec succèe!!</div><?php
              }else{?>
          ...
          if (!isset($_GET['ajoutdep'])) {?>
            <table class="table table-hover table-bordered table-striped table-responsive text-center">
              <thead class="sticky-top bg-light text-center">
                <tr><th colspan="6"><?="Liste des Remboursements " .$datenormale ?> <a href="persremboursement.php?ajoutdep" class="btn btn-warning">Enregistrer un Remboursement</a></th></tr>

                <tr>
                  <form class="form" method="POST" action="persremboursement.php" id="suitec" name="termc">
              ...
              <tbody><?php 
                ...
                if (isset($_POST['j1'])) {    
                  ...
                  $products= $DB->query("SELECT *FROM persremboursement WHERE DATE_FORMAT(date_payement, \"%Y%m%d\")>='{$_SESSION['date1']}' and DATE_FORMAT(date_payement, \"%Y%m%d\")<='{$_SESSION['date2']}' order by(id) desc");

                }else{
                  $products= $DB->query("SELECT *FROM persremboursement where lieuvente='{$_SESSION['lieuvente']}' order by(id) desc");              
                }
                $montantgnf=0;
                $montantgnfav=0;
                $montantgnfpr=0;
                ...
                foreach ($products as $keyv=> $product ){
                  ...
                  <tr>
                    ...
                    <td><?=$panier->nomPersonnelId($product->client);?></td>
                    ...
                    <td><?php if ($_SESSION['level']>=6){?><a class="btn btn-danger" href="persremboursement.php?deleteret=<?=$product->numdep;?>" onclick="return alerteS();">Supprimer</a><?php };?></td>
      ...
      $(document).ready(function(){
          $('#search-user').keyup(function(){
              ...
              if (utilisateur!='') {
                  $.ajax({
                      type: 'GET',
                      url: 'recherche_utilisateur.php?persremboursement',
