...
class commande {
    ...
    public function dernierPrixAchatCmd($idprod){
        $prodmax=$this->DB->querys("SELECT max(id) as max from gestionachatfournisseur where idprod='{$idprod}'");
        $prod=$this->DB->querys("SELECT *from gestionachatfournisseur where id='{$prodmax['max']}'");

        return $prod;
    }

    public function nbreCmdConfirmeByIdprod($idprod){
        $statut="nok";
        $prod=$this->DB->querys("SELECT COUNT(id) as nbre from gestionachatfournisseur where idprod='{$idprod}' and statut='{$statut}' ");

        return $prod;
    }

    public function nbreCmdRecuByIdprodByBoutique($idprod, $lieuvente, $libelle){
        $prod=$this->DB->querys("SELECT SUM(quantitemouv) as qtite from stockmouv where idstock='{$idprod}' and lieuvente_mouv='{$lieuvente}' and   numeromouv = '{$libelle}' ");
        return $prod;
    }

    public function nbreAchatRecuByIdprodByBoutique($idprod, $lieuvente, $libelle){
        $prod=$this->DB->querys("SELECT SUM(quantitemouv) as qtite from stockmouv where idstock='{$idprod}' and lieuvente_mouv='{$lieuvente}' and   numeromouv = '{$libelle}' ");
        return $prod;
    }
