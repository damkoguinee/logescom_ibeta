...
foreach ($sql_annees as $row_annees) {
    ...
    $sql_ventes_par_mois = $DB->query("SELECT MONTH(date_cmd)as MOIS, SUM(Total) AS PRODUIT_VENTE FROM payement where num_client='{$numclient}' and year(date_cmd)='{$row_annees->ANNEE}' GROUP BY MOIS");
...
// TITRE: texte
$graph->title->Set("Totaux des ventes de ".$_SESSION['nomclientsuivi']);
