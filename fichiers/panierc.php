<div class="cmd"><?php
	...
	if (isset($_GET['delpc'])) {
		...
		$DB->delete('DELETE FROM validcomande WHERE id_produit = ? and pseudo=?', array($_GET['delpc'], $_SESSION['idpseudo']));
	}

	if (isset($_GET['desig'])) {

		$prodvalidcverif = $DB->querys('SELECT quantite FROM validcomande where id_produit=? and pseudo=?', array($_GET['idc'], $_SESSION['idpseudo']));
	...
	if (isset($_GET['scanneurc'])) {
		...
		$prodvalidcverif = $DB->querys('SELECT quantite FROM validcomande where id_produit=? and pseudo=?', array($prodstock['id'], $_SESSION['idpseudo']));

		if (empty($prodvalidcverif)) {
					
			$DB->insert('INSERT INTO validcomande (id_produit, codebvc, designation, quantite, pachat, pvente, previent, frais, etat, pseudo, datecmd) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, now())', array($prodstock['id'], $_GET['scanneurc'], $prodstock['designation'], 1, $prodstock['prix_achat'], $prodstock['prix_vente'], 0, 0, 'paye', $_SESSION['idpseudo']));

		}else{

			$qtitesup=$prodvalidcverif['quantite']+1;

			$DB->insert('UPDATE validcomande SET quantite=? where id_produit=? and pseudo=?', array($qtitesup, $prodstock['id'], $_SESSION['idpseudo']));
	...
	if (isset($_POST['modifcom']) or isset($_GET['modifcom'])) {
		...
		$DB->insert('UPDATE validcomande SET quantite=?, pachat=?, frais=?, pvente=? where id_produit=? and pseudo=?', array($_POST['quantity'], $_POST['pachat'], $_POST['frais'], $_POST['pvente'], $_POST['id'], $_SESSION['idpseudo']));
	}

	$prodvalidc = $DB->query("SELECT idprod as id, id_produit, validcomande.quantite as quantite, validcomande.designation as designation, pvente, pachat, prix_achat, prix_vente, frais, date_format(dateperemtion, \"%d/%m/%Y \") as dateperemtion FROM validcomande inner join `".$nomtab."` on idprod=validcomande.id_produit where pseudo='{$_SESSION['idpseudo']}' order by(validcomande.id)desc");
	...
	if (!empty($prodvalidc)) {?>
			...
			foreach($prodvalidc as $key=> $product){
				...
				<form id="modifcom" action="commande.php?modifcom" method="POST">
					...
					<tbody>
						...
						<td><input  type="text" min="0" name="quantity" value="<?=$product->quantite;?>"></td><?php
						...
						<td class="supc">
							<a onclick="return alerteV();" href="commande.php?delpc=<?= $product->id_produit; ?>" class="del"><img src="img/sup.jpg"></a>
						</td>
		...
		<form id="naissance" method="POST" action="commande.php" style="margin-top: 0px; margin-top:10px;" >

			<fieldset><legend></legend>
	      <ol>
	      	<li><label>N°Fact/BL</label><input type="text" min="0"  name="numfact" required=""></li>
	      	<li><label>Date Fact/BL</label><input type="date"  name="datefact" required=""></li>
	      ...
	      <ol style="background-color:orange;">
	        ...
	        <li><label>Frais de Douane</label><input style="width: 150px; font-size: 22px; " type="text" min="0"  name="fraisd" >
	        	<input style="width: 150px; font-size: 22px; " type="hidden" name="fraisdp" value="0" min="0">

	        	Société de douane
	        	<select  type="text" name="clientd">
	            <option></option><?php
	            $type1='douanier';
				$type2='douanier';
				foreach($panier->clientF($type1, $type2) as $product){?>
	        ...
	        <li><label>Autres Frais</label><input style="width: 150px; font-size: 22px; " type="text" min="0"  name="fraisa" value="0" >
	        </li>
	      </ol>
	    </fieldset>
		<fieldset>
