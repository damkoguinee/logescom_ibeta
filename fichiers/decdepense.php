              ...
              if ($_POST['montant']<0){?>
                ...
                if (!empty($_POST['montant']) and !empty($_POST['categorie']) and !empty($_POST['compte'])) {
                  ...
                  if (empty($dateop)) {                        
                    ...
                    $DB->insert('INSERT INTO banque (id_banque,origine,typeent,montant,libelles, numero, devise, lieuvente, date_versement) VALUES(?,?,?, ?, ?, ?, ?, ?, now())', array($compte,"depense","depense", -$montant, $motif, 'retd'.$numdec, $devise, $lieuventeret));
                  }else{
                    ...
                    $DB->insert('INSERT INTO banque (id_banque,origine,typeent,montant,libelles, numero, devise, lieuvente, date_versement) VALUES(?,?,?, ?, ?, ?, ?, ?, ?)', array($compte,"depense","depense", -$montant, $motif, 'retd'.$numdec, $devise, $lieuventeret, $dateop));
                  }?>
