	...
	class panier{
		...
		private $DB;
		public $monnaie=['gnf','aed','us', 'eu'];

		public $modep=['espèces','chèque','virement', 'différé'];
		public $modep1=['especes','cheque','virement'];
		...
		public $email=['d.amadoumouctar@yahoo.fr'];

		public $delai=30;
		public $delaialerte=15;
		public $estimationpr=8;
      ...
      foreach ($mode_payement as $key=> $produc ){
		...
		public function decaissementBil($modep, $type, $date1, $date2){
			...
			if (isset($_POST['magasin']) and $_POST['magasin']=='general') {
				...
				$prodcmd=$this->DB->querys("SELECT sum(montant) as montant FROM banquecmd where devise='{$modep}' and payement='{$type}' and DATE_FORMAT(dateop, \"%Y%m%d\") >='{$date1}' and DATE_FORMAT(dateop, \"%Y%m%d\") <= '{$date2}'");
				...
				$prodcmd=$this->DB->querys("SELECT sum(montant) as montant FROM banquecmd where devise='{$modep}' and payement='{$type}' and lieuvente='{$_SESSION['lieuvente']}' and DATE_FORMAT(dateop, \"%Y%m%d\") >='{$date1}' and DATE_FORMAT(dateop, \"%Y%m%d\") <= '{$date2}'");
			...
			if ($modep=='gnf') {
				return ($prod['montant']+$prodcmd['montant']+$prodep['montant']+$prodfraisup['montant']+$prodfraismarch['montant']+$prodfourn['montant']);
			}else{
				return ($prod['montant']+$prodcmd['montant']+$prodep['montant']+$prodfraisup['montant']+$prodfourn['montant']);
			}
