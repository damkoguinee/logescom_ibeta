...
$sql_annees = $DB->query("SELECT YEAR(date_cmd)as ANNEE FROM payement where num_client='{$numclient}' GROUP BY ANNEE");
...
foreach ($sql_annees as $row_annees) {
    ...
    $sql_ventes_par_mois = $DB->query("SELECT MONTH(date_cmd)as MOIS, COUNT( `id` ) AS NOMBRE_VENTE FROM payement where num_client='{$numclient}' and year(date_cmd)='{$row_annees->ANNEE}' GROUP BY MOIS");

    foreach ($sql_ventes_par_mois as $row_mois) {

        $tableauVentesParAnnees[$row_annees->ANNEE][$row_mois->MOIS-1] = $row_mois->NOMBRE_VENTE;
    }
}
...
// Fixer les marges
$graph->img->SetMargin(100,10,30,40);
...
// TITRE: texte
$graph->title->Set("Nombre des ventes par mois de ".$_SESSION['nomclientsuivi']);
