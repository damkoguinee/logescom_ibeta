        ...
        <table class="table table-hover table-bordered table-striped table-responsive text-center align-middle">
          ...
          <tbody><?php
            if (isset($_GET['search'])) {
              ...
              $products= $DB->query("SELECT *FROM achat_fournisseur inner join client on id_client=client.id  WHERE (nom_client LIKE ? OR telephone LIKE ? OR bl LIKE ? ) and  lieuvente LIKE ? order by(achat_fournisseur.id) DESC ", array("%".$terme."%", "%".$terme."%", "%".$terme."%", $_SESSION['lieuvente']));
            }elseif (isset($_GET['search_fact'])) {
              $terme = htmlspecialchars($_GET["search_fact"]);
              $products= $DB->query("SELECT *FROM achat_fournisseur  WHERE (id_client LIKE ?) and  lieuvente LIKE ? order by(achat_fournisseur.id) DESC ", array($terme, $_SESSION['lieuvente']));
            }else{
              if ($_SESSION['level']>6) {
                $products= $DB->query("SELECT *FROM achat_fournisseur  order by(id)");
              }else{
                $products= $DB->query("SELECT *FROM achat_fournisseur  WHERE lieuvente='{$_SESSION['lieuvente']}' ");
              } 
            }   
