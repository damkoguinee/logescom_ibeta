            ...
            if (isset($_POST['valid'])){
              ...
              if (empty($_POST["client"]) OR empty($_POST["montant"]) or empty($_POST['devise'])) {?>
                ...
                if ($_POST['montant']!="") {
                  ...
                  $DB->insert('INSERT INTO banque (id_banque, clientbanque, montant, libelles, numero, devise, lieuvente, typeent,origine, numeropaie, banqcheque, date_versement) VALUES(?, ?, ?, ?, ?, ?,?, ?, ?, ?, ?, ?)', array($compte, $client, -$montant, $motif, 'ret'.$numdec, $devise, $lieuventeret, "retrait","client", $numcheque, $banquecheque, $dateop));
  
                  if ($prodclient['typeclient']=='Banque') {
  
                    $DB->insert('INSERT INTO banque (id_banque, clientbanque, montant, libelles, numero, devise, lieuvente, typeent,origine, numeropaie, banqcheque, date_versement) VALUES(?, ?, ?, ?, ?, ?, ?,?, ?, ?, ?, ?)', array($prodclient['id'], $client, $montant, $motif, 'ret'.$numdec, $devise, $lieuventeret, "retrait","client", $numcheque, $banquecheque, $dateop));
