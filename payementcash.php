...
$prodvente = $DB->querys("SELECT *FROM validvente_cash where pseudop='{$pseudo}' ");
...
if (is_null($_SESSION['clientvip']) and is_null($_SESSION['clientvipcash'])) {
	...
	if (isset($_POST['proformat'])) {
		...
		$products= $DB->query("SELECT id_produit as id, validpaie_cash.quantite as qtite, pvente FROM validpaie_cash where pseudov='{$_SESSION['idpseudo']}' order by(validpaie_cash.id)");
		...
		foreach($products as $product){
			...
			//$DB->delete('DELETE FROM validpaie_cash');
			//$DB->delete('DELETE FROM validvente_cash');
		...
		$products= $DB->query("SELECT id_produit as id, productslist.designation as designation, validpaie_cash.quantite as qtite, pvente, Marque, nbrevente, type FROM validpaie_cash inner join productslist on productslist.id=id_produit where pseudov='{$pseudo}' order by(validpaie_cash.id)");
		...
		foreach($products as $product){		
			...
			if (!empty($_POST['stockd'])) {
				...
				if ($quantite>0) {
					...
					$DB->insert('INSERT INTO stockmouv (idstock, origine, client, numeromouv, libelle, quantitemouv, idnomstock, dateop) VALUES(?, ?, ?, ?, ?, ?, ?, now())', array($liaison, 'vente cash', $numclient, 'ouvc'.$init.$numero_commande, 'sortie', -1, $idstock)); 

					$DB->insert('INSERT INTO stockmouv (idstock, origine, client, numeromouv, libelle, quantitemouv, idnomstock, dateop) VALUES(?, ?, ?, ?, ?, ?, ?, now())', array($id, 'vente cash', $numclient, 'ouvc'.$init.$numero_commande, 'entree', $products["qtiteintp"], $idstock));
				...
				if ($quantite>0) {
					...
					if ($productsp["quantite"]>0) {
						...
						if ($products["quantite"]>0) {
							...
							$DB->insert('INSERT INTO stockmouv (idstock, origine, client, numeromouv, libelle, quantitemouv, idnomstock, dateop) VALUES(?, ?, ?, ?, ?, ?, ?, now())', array($id,  "vente cash", $numclient,'ouvc'.$init.$numero_commande, 'entree', $products["qtiteintd"], $idstock));
				...
				$DB->insert('INSERT INTO stockmouv (idstock, origine, client, numeromouv, libelle, quantitemouv, idnomstock, dateop) VALUES(?, ?, ?, ?, ?, ?, ?, now())', array($id, "vente cash", $numclient, 'liv'.$init.$numero_commande, 'sortie', -$qtiteliv, $idstock));  
		...
		if (($panier->espace($_POST['reste'])+$fraisup)<=0) {
			...
			if (!empty($cheque) or !empty($eu) or !empty($us) or !empty($cfa)) {
				...
				$insert->insertPayementCash($init.$numero_commande,"vente cash", $total, $fraisup, $montantreliquat, $remise, $reste, $etat,'nonlivre', $pseudo, $numclient, $_SESSION['clientvipcash'], $caisse, $lieuventecaisse, $datev,$datealerte);
				...
				$insert->insertPayementCash($init.$numero_commande,"vente cash", $total, $fraisup, $montantpaye, $remise, $reste, $etat,'nonlivre', $pseudo, $numclient, $_SESSION['clientvipcash'], $caisse, $lieuventecaisse, $datev,$datealerte);
		...
		$DB->delete("DELETE FROM validpaie_cash where pseudov='{$pseudo}' ");
		$DB->delete("DELETE FROM validvente_cash where pseudop='{$pseudo}' ");
		...
		header("Location: ventedirect.php?payement");
