              ...
              if (isset($_POST['valid'])){            
                ...
                if ($_POST['montant']<0){?>
                  ...
                  if (!empty($_POST['montant']) and !empty($_POST['categorie']) and !empty($_POST['compte'])) {
                    ...
                    $DB->insert('INSERT INTO recette (numdec, categorie, montant, devisedep, payement, coment, cprelever, lieuvente, compte_op, idpers, date_payement) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?,  ?)',array('depr'.$numdec, $categorie, $montant, $devise, $payement, $motif, $compte, $lieuventeret, $compte_op, $_SESSION['idpseudo'], $dateop));

                    $DB->insert('INSERT INTO banque (id_banque, clientbanque,typeent,origine,montant, libelles, numero, devise, lieuvente, date_versement) VALUES(?,?,?,?, ?, ?, ?, ?, ?, ?)', array($compte,"","recette",$compte_op_nom, $montant, $motif, 'depr'.$numdec, $devise, $lieuventeret, $dateop));?>
