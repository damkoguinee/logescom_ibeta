            ...
            if (empty($_POST["client"]) OR empty($_POST["montant"]) or empty($_POST['devise']) OR empty($_POST["motif"])) {?>
              ...
              if ($motif=='achat devise') {
                ...
                if (($convert)>$panier->montantCompteBil($compte, 'gnf')) {?>
                  ...
                  $DB->insert('INSERT INTO banque (id_banque,typeent,origine, montant, libelles, numero, devise, lieuvente, date_versement) VALUES(?,?,?, ?, ?, ?, ?, ?, ?)', array($compte,'echange','echange', $montant, $client, 'devisea'.$max, $devise, $_SESSION['lieuvente'], $dateop));
  
                  $DB->insert('INSERT INTO banque (id_banque,typeent,origine, montant, libelles, numero, devise, lieuvente, date_versement) VALUES(?,?,?, ?, ?, ?, ?, ?, ?)', array($compte,'echange','echange', -$convert, $client, 'devisea'.$max, 'gnf', $_SESSION['lieuvente'], $dateop));?>
                ...
                if (($montant)>$panier->montantCompteBil($compte, $devise)) {?>
                  ...
                  $DB->insert('INSERT INTO banque (id_banque,typeent,origine, montant, libelles, numero, devise, lieuvente, date_versement) VALUES(?,?,?, ?, ?, ?, ?, ?, ?)', array($compte,'echange','echange', -$montant, $client, 'devisev'.$max, $devise, $_SESSION['lieuvente'], $dateop));
  
                  $DB->insert('INSERT INTO banque (id_banque,typeent,origine, montant, libelles, numero, devise, lieuvente, date_versement) VALUES(?,?,?, ?, ?, ?, ?, ?, ?)', array($compte,'echange','echange', $convert, $client, 'devisev'.$max, 'gnf', $_SESSION['lieuvente'], $dateop));?>
                  <div class="alert alert-success">Opération éffectuée avec succèe!!!</div><?php
                }
          ...
          if (isset($_GET['ajoutdev']) or isset($_POST['taux'])) {?>
            <form class="form" id="naissance" method="POST">
              ...
              if (isset($_POST['taux'])) {
                ...
                <label class="form-label">Montant Devise*</label><input class="form-control" type="text" name="montant" value="<?=$panier->formatNombre($montant);?>" onchange="this.form.submit()" required=""><input class="form-control" type="hidden" name="motif" value="<?=$_SESSION['motifdev'];?>">
                ...
                <label class="form-label">Montant Devise*</label><input class="form-control" type="number" name="montant" min="0" required="" placeholder="entrer le montant en devise"><input class="form-control" type="hidden" name="motif" value="<?=$_SESSION['motifdev'];?>">
