          ...
          if (isset($_GET['listep']) or isset($_POST['validp']) or !empty($_SESSION['resultidprodlp'])) {?>
            ...
            <table class="table table-hover table-bordered table-striped table-responsive text-center">		
              <thead>
                ...
                <tr>
                  <th class="legende" colspan="5" height="30">Liste des Pertes <a class="btn btn-warning" href="pertes.php?ajoutpertes&nomstock=<?=$_SESSION['idnomstock'];?>" >Ajouter une Perte</a></th>

                  <th colspan="2">

                    <input class="form-control"  id="search-userlp" type="text" name="clientsearchlp" placeholder="rechercher un produit" />
                    <div class="bg-danger" id="result-searchlp"></div>
                    </th>
                </tr>
              ...
              <tbody><?php 
                ...
                if (!empty($_SESSION['resultidprodlp'])) {                 
                  ...
                  if ($_SESSION['nomstock']=='stock general') {
                    ...
                    $products= $DB->query("SELECT idnomstockp, pertes.id as id, idpertes, Marque as designation, motifperte as motif, pertes.prix_revient as prix_revient, pertes.quantite as quantite, datepertes FROM pertes inner join productslist on productslist.id=idpertes  WHERE YEAR(datepertes)='{$date}' order by(idpertes)");
                  }else{
                ...
                foreach ($products as $key=> $product ){
                  ...
                  <tr>
                    ...
                    <td class="text-start"><?=$panier->nomPertes($product->motif)[0];?></td>

                    <td class="text-start"><?= ucwords(strtolower($product->designation)); ?></td>

                    <td><?= $product->quantite; ?></td> 

                    <td class="text-end"><?= number_format($totrevient,0,',',' '); ?></td>

                    <td><?= (new dateTime($product->datepertes))->format('d/m/Y'); ?></td>

                    <td><?php if ($_SESSION['level']>6){?><a class="btn btn-danger" onclick="return alerteS();" href="pertes.php?deleteret=<?=$product->idpertes;?>&id=<?=$product->id;?>&qtite=<?=$product->quantite;?>&stock=<?=$product->idnomstockp;?>">Annuler</a><?php };?></td>
                  </tr><?php 
                }?>
              ...
              <tfoot>
                  <tr>
                    ...
                    <th><?= $cumulqtite;?></th>
                    <th class="text-end"><?= number_format($cumulmontant,0,',',' ');?></th>
                  </tr>
              </tfoot>
