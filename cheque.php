          ...
          <tbody>
            ...
            <td>                           
              ...
              <select style="width:200px;" type="text" name="client" required=""><?php
                ...
                if ($_SESSION['lieuvente']!=1 or $_SESSION['level']>6) {?>
                  <option value="centrale" style="font-size: 18px;">Envoi Caisse Centrale</option><?php 
                }?>
      ...
      if ($_POST['montant']!="") {
        ...
        if (empty($dateop)) {
          ...
          if ($prodclient['typeclient']!='Banque') {
            ...
            if ($client!='liquidite' and $client!='centrale') {
          ...
          if ($client=='centrale') {
            ...
            $DB->insert('INSERT INTO banque (id_banque, montant, libelles, numero, typeent, devise, typep, lieuvente, numeropaie, date_versement) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, now())', array(1, $montant, "transfcheque (".$motif.')', 'ret'.$numdec, 'transfert cheque', 'gnf', 'cheque', 1, $numcheque));

          }

          if ($prodclient['typeclient']=='Banque') {

            $DB->insert('INSERT INTO banque (id_banque, montant, libelles, numero, devise, typep, lieuvente, numeropaie, date_versement) VALUES(?, ?, ?, ?, ?, ?, ?, ?, now())', array($client, $montant, "Retrait (".$motif.')', 'ret'.$numdec, $devise, 'especes', $_SESSION['lieuvente'], $numcheque));
          ...
          if ($client=='centrale') {
            ...
            $DB->insert('INSERT INTO banque (id_banque, montant, libelles, numero, typeent, devise, typep, lieuvente, numeropaie, date_versement) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?)', array(1, $montant, "transfcheque (".$motif.')', 'ret'.$numdec, 'transfert cheque', 'gnf', 'cheque', 1, $numcheque, $dateop));

          }

          if ($prodclient['typeclient']=='Banque') {

            $DB->insert('INSERT INTO banque (id_banque, montant, libelles, numero, devise, typep, lieuvente, numeropaie, date_versement) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?)', array($client, $montant, "Retrait (".$motif.')', 'ret'.$numdec, $devise, 'especes', $_SESSION['lieuvente'], $numcheque, $dateop));
        ...
        if ($client=='centrale') {
          ...
          //$DB->insert('INSERT INTO transferfond (numero, caissedep, montant, caisseret, devise, exect, lieuvente, coment, dateop) VALUES(?, ?, ?, ?, ?, ?, ?, ?, now())', array('ret'.$numdec, 1, $montant, $compte, $devise, $_SESSION['idpseudo'],  $_SESSION['lieuvente'], $motif));

          $DB->insert("UPDATE modep SET etatcheque = ?, caisse=? WHERE numerocheque = ?", array('non traite', 1, $numcheque));

          if ($datefact!=$datejour) {
            $DB->insert('INSERT INTO chequedepasse (numcheque, montant, dateencaissement, id_banque, lieuvente, dateop) VALUES(?, ?, ?, ?, ?, now())',array($numcheque, $montant, $_POST['datefact'], 1, 1));
          }
      ...
      <tbody><?php 
        ...
        if ($_SESSION['level']>6) {
          ...
          if (isset($_POST['j1'])) {
            ...
            $products= $DB->query("SELECT numpaiep, numdec, modep.id as id, modep.montant as montant, modep.client as client, decaissement.client as clientrecep, decaissement.date_payement as datesortie, numerocheque, etatcheque, modep.banquecheque as banquecheque, datefact, caisse FROM modep left join decaissement on numerocheque=numcheque WHERE modep='{$cheque}' order by(etatcheque)");
          }
        }else{
          ...
          if (isset($_POST['j1'])) {
            ...
            $products= $DB->query("SELECT numpaiep, numdec, modep.id as id, modep.montant as montant, modep.client as client, decaissement.client as clientrecep, decaissement.date_payement as datesortie, numerocheque, etatcheque, modep.banquecheque as banquecheque, datefact, caisse FROM modep left join decaissement on numerocheque=numcheque WHERE caisse='{$caisse}' and  modep='{$cheque}' order by(etatcheque)");
          }
        }
