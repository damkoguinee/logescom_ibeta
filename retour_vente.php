...
<div class="container-fluid mt-3">
    <div class="row"><?php 
        ...
        if (isset($_POST['updateMontant'])) {
            ...
            $DB->insert("UPDATE payement SET Total='{$new_montant_facture}', montantpaye='{$new_montant_paye}' where num_cmd='{$numcmd}'");
            ...
            $DB->insert("INSERT INTO commande (id_produit, prix_vente, prix_achat, prix_revient, quantity, qtiteliv, etatlivcmd, num_cmd, id_client) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?) ", array($idprod, $pvente, $pa, $pr, -$qtite, -$qtite, "livre", $numcmd, $client));

            $numero_mouv="liv".$numcmd;
            

            $DB->insert("INSERT INTO stockmouv (idstock, numeromouv, libelle, quantitemouv, idnomstock, idpers) VALUES(?, ?, ?, ?, ?, ?) ", array($idprod, $numero_mouv, 'entree', $qtite, $idStock, $_SESSION['idpseudo']));
