          ...
          <div class="row" >
            ...
            <table class="table table-bordered table-striped table-hover align-middle">
                ...
                <tr>
                    ...
                    <th>Prix Revient</th>
                    <th>
                      <label for="">Bénéfice</label><br/>
                      <label class="bg-info p-1" for=""><?=$configuration->formatNombre($caisse->cumulPayement("vente credit",$_SESSION['client_credit'], $_SESSION['date1'], $_SESSION['date2'])['benefice']);?></label>
                    </th>
                </tr>
                </thead>
                <tbody><?php
                ...
                foreach ($products as $key=> $product ){
                  if ($product->quantity<0) {
                    $etat_fact="retour vf";
                  }elseif ($product->etat=='totalite') {
                    $etat_fact="fact payé";
                  }else{
                    $etat_fact="fact impayé";
                  }

                    $cumul_com+=$product->quantity*$product->prix_vente-$product->prix_revient*$product->quantity;?>

                    <tr>
                    <td class="text-center"><?=$key+1;?></td>
                    <td class="text-center"><?=$etat_fact;?></td>
                    ...
                    <td class="text-end"><?=$configuration->formatNombre($product->quantity*$product->prix_vente-$product->prix_revient*$product->quantity); ?></td>
                    </tr><?php 
                } ?>   
                </tbody>
                ...
                <tr>
                    <th colspan="10"></th>
                    <th class="text-end"><?= $configuration->formatNombre($cumul_com);?></th>
                </tr>
                </tfoot>
            </table>
          </div>
  ...
  $(document).ready(function(){
      $('#search-user').keyup(function(){
          ...
          if (utilisateur!='') {
              $.ajax({
                  type: 'GET',
                  url: 'recherche_utilisateur.php?produit_client',
